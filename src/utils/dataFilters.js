...
export const filterHotelsByDate = (hotels, startDate, endDate, nights) => {
    ...
    filteredHotels = hotels.filter(hotel => {
        ...
        let yes =  availability.filter(availabilityDate => {
          ...
          if (availabilityTo.isBetween(startDate, endDate, 'days', '[]') && availabilityFrom.isBetween(startDate, endDate, 'days', '[]')) {
            return true;
          }
          return false;
         });
...
export const findRangeValues = (hotels) =>{
    ...
    const max = sortedArray.length > 0 ? sortedArray[sortedArray.length -1].price : 0;
